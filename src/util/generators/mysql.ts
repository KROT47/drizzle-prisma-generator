  ...
  switch (type.toLowerCase()) {
    ...
    case 'bytes':
      ...
      throw new GeneratorError(
        "Drizzle ORM doesn't support binary data type for MySQL"
      );
    case 'datetime':
...
export const generateMySqlSchema = (options: GeneratorOptions) => {
  ...
  for (const schemaTable of modelsWithImplicit) {
    ...
    const columnFields = Object.fromEntries(
      schemaTable.fields
        .map((e) => [
          e.name,
          prismaToDrizzleColumn(e, enums as UnReadonlyDeep<typeof enums>),
        ])
  ...
  const mySqlImportsArr = Array.from(mySqlImports.values()).sort((a, b) =>
    a.localeCompare(b)
  );
  ...
  const output = [importsStr, ...tables, ...rqb]
    .filter((e) => e !== undefined)
    .join('\n\n');
